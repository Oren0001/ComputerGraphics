...
RayHit intersectScene0(Ray ray)
{
    ...
    intersectTriangle(ray, bestHit, mirrorHalf, p1, p2, p0, true);
...
// Defines a scene with circles and cylinders
RayHit intersectScene5(Ray ray)
{
    Material floor = CreateMaterial(float3(0.95, 0.85, 0.7), 0.15);
    Material diffuse = CreateMaterial(float3(0.14, 0.43, 0.84), 0);
    Material gold = CreateMaterial(0, float3(1.0f, 0.78f, 0.34f));

    RayHit bestHit = CreateRayHit();
    intersectPlane(ray, bestHit, floor, float3(0, 0, 0), float3(0, 1, 0));
    intersectCircle(ray, bestHit, gold, float4(0, 1, 0, 0.5), float3(0, 0, 1), true);

    float n = 20.0;
    for (int i = 0; i < n; i++)
    {
        float angle = 2 * PI * (i / n);
        intersectCylinderY(ray, bestHit, diffuse, float4(2*sin(angle), 1.25 + sin(_Time.y + 2*angle), 2*cos(angle), 0.1), 0.5);
    }
    return bestHit;
}
